...
interface EditBookModalProps {
  book: Book;
  onClose: () => void;
  onSave: (data: Omit<Book, "id" | "chapters">) => void;
  loading?: boolean;
}

type LookupState = "idle" | "searching" | "found" | "notfound";

export function EditBookModal({ book, onClose, onSave, loading = false }: EditBookModalProps) {
  const [title, setTitle] = useState(book.title);
  const [author, setAuthor] = useState(book.author);
  const [grade, setGrade] = useState<string>(String(book.grade));
  ...
  const canSave = title.trim() && author.trim() && grade && !loading;
  const hasChanges =
    title.trim() !== book.title ||
    author.trim() !== book.author ||
    Number(grade) !== book.grade;
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input
            ...
            placeholder={lookupState === "searching" ? "Searching..." : "Author name"}
