...
- **Key files**:
  ...
  - `src/standards/` — CA CCSS ELA standards system (grades 3–8, RL/RI/W only; pure TypeScript, no runtime PDF): `types.ts` (ELAStandard, FolderStandardsProfile, GradeLevel, StandardStrand, CurriculumUse), `elaStandards.ts` (all standard objects), `index.ts` (getStandardsForGrade, getStandardsForFolder, createStandardsProfileForFolder, formatStandardsForPrompt). `POST /api/books` now returns a `standardsProfile` field. The `standards` teacher-guide section prompt is injected with grade-specific real standard codes from this module.
